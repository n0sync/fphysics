# Mathematical Topics
##### A collection of fascinating math theories that I find intriguing and worth exploring.
##### Mentions: Most of these concepts were introduced to me through videos from Veritasium and 3Blue1Brown.

## Analysis & Calculus
* [Taylor Series](https://en.wikipedia.org/wiki/Taylor_series)
* [Laplace Transform](https://en.wikipedia.org/wiki/Laplace_transform)
* [Plancherel Theorem](https://en.wikipedia.org/wiki/Plancherel_theorem)

## Number Theory
* [Basel Problem](https://en.wikipedia.org/wiki/Basel_problem)
* [Riemann Zeta Function](https://en.wikipedia.org/wiki/Riemann_zeta_function)
* [Goldbach Conjecture](https://en.wikipedia.org/wiki/Goldbach%27s_conjecture)
* [Dirichlet's Theorem](https://en.wikipedia.org/wiki/Dirichlet%27s_theorem_on_arithmetic_progressions)
* [P-adic Numbers](https://en.wikipedia.org/wiki/P-adic_number)

## Algebra & Polynomials
* [Hermite Polynomials](https://en.wikipedia.org/wiki/Hermite_polynomials)
* [Depressed Cubic](https://en.wikipedia.org/wiki/Cubic_equation#Depressed_cubic)

## Linear Algebra & Functional Analysis
* [Eigenfunctions and Eigenvalues](https://en.wikipedia.org/wiki/Eigenfunction)
* [Tensors](https://en.wikipedia.org/wiki/Tensor)
* [Kronecker Delta](https://en.wikipedia.org/wiki/Kronecker_delta)
* [Schwarz Inequality](https://en.wikipedia.org/wiki/Cauchy%E2%80%93Schwarz_inequality)

## Probability & Statistics
* [Law of Large Numbers](https://en.wikipedia.org/wiki/Law_of_large_numbers)
* [Markov Chain](https://en.wikipedia.org/wiki/Markov_chain)
* [Bayes' Theorem](https://en.wikipedia.org/wiki/Bayes%27_theorem)
* [Buffon's Needle](https://en.wikipedia.org/wiki/Buffon%27s_needle_problem)

## Mathematical Physics & Applications
* [Pi Collision Problem](https://en.wikipedia.org/wiki/Block-stacking_problem#Pi_from_collisions)
* [Black-Scholes-Merton Model](https://en.wikipedia.org/wiki/Black%E2%80%93Scholes_model)

## Logic & Foundations
* [Gödel's Incompleteness Theorems](https://en.wikipedia.org/wiki/G%C3%B6del%27s_incompleteness_theorems)
* [Hilbert's Program](https://en.wikipedia.org/wiki/Hilbert%27s_program)
* [Types of Infinity](https://en.wikipedia.org/wiki/Infinity#Mathematics)
* [Halting Problem](https://en.wikipedia.org/wiki/Halting_problem)

## Geometry & Topology
* [Inscribed Square Problem](https://en.wikipedia.org/wiki/Inscribed_square_problem)
